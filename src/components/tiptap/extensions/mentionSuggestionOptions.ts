...
import { useGetUserForSpacesQuery } from "../../../services/space";

import { SpaceUsersFromApi } from "../../../services/space";
...
export const mentionSuggestionOptions: MentionOptions["suggestion"] = {
  ...
  items: async ({query}) =>{
    ...
    const arrayOfVals = url.split('/') 
    const spaceName = arrayOfVals[4]
    const res = await fetch(`http://localhost:8000/spaces/users/${spaceName}/` ,{
        credentials:'include'
    })
    const data = await res.json() as SpaceUsersFromApi
    const usersToTag = [...data?.users , data?.creater]
    return(
     usersToTag
        .map((user, index) => (
            user as MentionSuggestion
        ))
        .filter((item) =>
